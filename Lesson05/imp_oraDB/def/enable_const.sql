...
--          dbms_output.put_line('CREATE USER '||item.sysname||' IDENTIFIED BY '||lower(item.sysname)|| ' PROFILE DEFAULT ACCOUNT UNLOCK;');
--          dbms_output.put_line('GRANT CREATE SESSION TO '||item.sysname||';');
        end if;
...
--          dbms_output.put_line('ALTER USER '||item.sysname||' ACCOUNT UNLOCK;');
      end if;
      i:=0;
      for it in (
...
loop
--  dbms_output.put_line('ALTER TABLE MTK.'||item.table_name||' DROP UNUSED COLUMNS;');
